...
function Button({ info }) {
  ...
  return (
    <div>
      <button
        onClick={() => navigate(`/notes/${buttonCount}`)}
        id={buttonID}
        class="focus:bg-slate-600 focus:text-white hover:bg-slate-500 w-full p-2"
      >
        <div class="font-bold text-xl float-left">{buttonTitle}</div>
        <div class="text-sm text-neutral-500 text-light float-left">
          {buttonDate}
        </div>
